                ...
                <div class="card">
                    ...
                    <div class="card-body card-block">
                        <form action=" {{route('categories.update', $category->id)}} " method="post"
                            ...
                            @if ($category->childrens->count() < 1) <div class="row form-group">
                                <div class="col col-md-2"><label for="select" class=" form-control-label">Parent
                                        Category</label></div>
                                <div class="col-12 col-md-9">
                                    <select name="parent_id" id="select" class="form-control">
                                        @foreach ($parentCategories as $item )
                                        <option value="{{$item->id}}" {{(old('parent_id') ?? $category->parent_id)
                                            == $item->id ? 'selected' : ''}} >
                                            {{$item->name}} </option>
                                        @endforeach
                                    </select>
